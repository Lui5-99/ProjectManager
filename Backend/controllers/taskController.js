...
const changeState = async (req, res) => {
  try {
    ...
    const taskStoraged = await Task.findById(id).populate("project").populate("complete");
    return res.status(200).json({ status: 200, message: "State change", data: taskStoraged });
  } catch (error) {
    return res.status(403).json({ status: 403, message: error.message });
  }
};
...
const deleteTask = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: 200, message: "Task deleted" });
  } catch (error) {
    const errorUser = new Error("Task not found");
    return res.status(404).json({ status: 404, message: errorUser.message });
  }
